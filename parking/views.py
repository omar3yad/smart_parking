...
class VehicleTrackingAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if best_match and max_combined_score >= SIMILARITY_THRESHOLD:
            ...
            # هذا سيجعل الـ JSON يظهر على عدة أسطر ومنظم
            logger.info(f"Response Sent:\n{json.dumps(response_data, ensure_ascii=False, indent=4)}")
            # ____________________________________
            return Response({
                "status": "Matched with High Confidence (>= 0.80) Omar's Algorithm",
                "identified_plate": best_match.license_plate,
...
class UpdateEntryEmbeddingAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # نفترض أن كاميرا الدخول ثابتة ومعروفة بـ CAM-ENTRY، ونستخدم last_seen بدلاً من entry_time لتكون أكثر مرونة مع تأخيرات المعالجة
...
class MyReservationsListAPIView(ListAPIView):
    """
    عرض كل حجوزات المستخدم الحالي (active + history).
    يستخدمه تطبيق الموبايل لعرض Booking History و Countdown.
    """
    permission_classes = [IsAuthenticated]
    serializer_class = ReservationSerializer

    def get_queryset(self):
        # تحرير الحجوزات المنتهية أولاً
        now = timezone.now()
        expired_res = Reservation.objects.filter(
        ...
        if expired_res.exists():
            slot_ids = list(expired_res.values_list('slot_id', flat=True))
            ParkingSlot.objects.filter(
                id__in=slot_ids,
                status='reserved'
            ).update(status='available')
        ...
        # رجع كل حجوزات المستخدم بترتيب تنازلي
